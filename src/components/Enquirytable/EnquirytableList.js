...
const EnquirytableList = () => {
  ...
  useEffect(() => {
    async function init() {
      const Enquiry = await inventoryApi.fetchEnquiries();
       //.log("Enquiry-->",Enquiry)
  ...
  const header = [
    {
      ...
      cell: (row) => (
        <div>
          <Link to={"/enquiry/" + row.id} state={row} className="name">
    ...
    { title: "Transactiontype", prop: "transactiontype", isFilterable: true },
    { title: "Verticals", prop: "verticals", isFilterable: true },
...
export default EnquirytableList;
